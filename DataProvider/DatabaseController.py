...
class DatabaseController():
    ...
    def add_new_user(self, user_name, room_id):
        ...
        self.rooms_collection.update_one({"_id": ObjectId(room_id)}, {"$push": {"users_list": insert_user.inserted_id}})
    ...
    def pick_random_black_card(self, room_id):  # Returns text and pick value of a random black card
        list_of_used_cards = self.rooms_collection.find_one({"_id": ObjectId(room_id)})["used_black_cards"]  # Find used black cards
        ...
        self.rooms_collection.update_one({"_id": ObjectId(room_id)}, {"$push": {"used_black_cards": result_card["_id"]}})
    ...
    def user_plays_cards(self, user_id, list_of_card_ids, room_id):
        ...
        for card in list_of_card_ids:
            self.rooms_collection.update_one({"_id":  ObjectId(room_id)}, {"$push": {"cards_on_table": ObjectId(card)}})
